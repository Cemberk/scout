...
Scout is built around a small `ContextProvider` abstraction — any source is just a subclass. Today ships providers for the **web**, **local filesystem**, **Slack**, **Google Drive**, and a **CRM** (the user's own contacts / projects / notes, stored in Postgres). GitHub, Gmail, Calendar, and a generic MCP wrapper land in the next release.
