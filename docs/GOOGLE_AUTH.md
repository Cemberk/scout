...
Verify: `GET http://localhost:8000/manifest` should show `drive` as `connected`, and the Navigator should be able to read email, list calendar events, and search Drive.
...
3. Scout's `GET /manifest` should show `drive` as `connected`. If it's `unconfigured`, the Google env vars aren't set.
