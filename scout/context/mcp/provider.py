...
Wraps a single Model Context Protocol (MCP) server as a Scout context
provider. One MCP server → one provider → one ``query_mcp_<slug>``
tool on Scout (via the sub-agent wrapper).

Why a sub-agent wrapper:

- Two MCP servers that each expose a ``search`` tool would collide if
  we flattened them onto Scout's tool list. The sub-agent namespace
  keeps each server's tool names isolated.
- Tool descriptions vary per server and change when the server updates.
  ...
  connect time, so Scout never sees stale hand-written tool docs.
- A crashed server degrades gracefully: ``status()`` flips to
  ``ok=False`` without taking down the agent.

Connection is lazy — the first ``query()`` / ``status()`` call pays
the ``initialize()`` cost. ``aclose()`` releases the session on
shutdown (awaited from ``app/main.py`` lifespan).
"""
...
from scout.context.provider import Answer, ContextProvider, Status, _sanitize_id
...
class MCPContextProvider(ContextProvider):
    ...
    def _default_tools(self) -> list:
        ...
        # tool name (e.g. `search`) would otherwise collide on Scout.
